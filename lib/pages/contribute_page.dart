...
class _ContributePageState extends ConsumerState<ContributePage> {
  ...
  Future<void> _loadPublicReports() async {
    ...
    final qualityReports = await contributeService.getPublicPathQualityReports();
  ...
  void _selectSearchResult(GeocodingResult result) async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not get current location for navigation.')));
      return; 
    }
    ...
    if (mounted && directions != null) {
        setState(() {
           ...
           _polylines.add(
             Polyline(
               ...
               color: isBikePath ? Colors.blue : Colors.grey, // Blue for bike, Grey for road
               width: 5,
             )
           );
        });
        ...
        // Zoom to fit route
        _mapController?.animateCamera(CameraUpdate.newLatLngBounds(directions.bounds, 50));
    } else if (mounted) {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not find a route to this location.')));
    }
  }
  ...
  void _onMapTap(LatLng position) {
    ...
    if (closestPoint != null && minDistance <= 50) {
      final snappedPosition = LatLng(closestPoint.latitude, closestPoint.longitude);
      setState(() {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please tap closer to the route line to report issues.')),
      );
    }
  }
    ...
    setState(() {
      ...
      _polylines = {
        Polyline(
          ...
          points: trip.points.map((p) => LatLng(p.latitude, p.longitude)).toList(),
  ...
  void _rateFullTrip(Trip trip) {
    ...
    final startPos = LatLng(trip.points.first.latitude, trip.points.first.longitude);
    ...
    showDialog(
      ...
      builder: (context) => _ReportPathQualityDialog(
        ...
        onSubmit: (report) async {
          await ref.read(contributeServiceProvider).addPathQualityReport(report);
          if (mounted) {
    ...
    showDialog(
      ...
      builder: (context) => _ReportObstacleDialog(
        ...
        onSubmit: (obstacle) async {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Obstacle reported!')));
            _loadPublicReports(); // Refresh map
          }
        },
      ),
        ...
        children: [
           ...
           Card(
             ...
             child: ListTile(
               ...
               onTap: () {
                 Navigator.push(context, MaterialPageRoute(builder: (_) => const BikePathFormPage()))
           ...
           const Padding(
             ...
             child: Align(alignment: Alignment.centerLeft, child: Text('OR Select a Recent Trip', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey))),
           ),
           Expanded(
             child: Consumer(
                   ...
                   data: (trips) {
                     return pathsAsync.when(
                       data: (paths) {
                          ...
                          combined.sort((a, b) {
                            DateTime timeA = a is Trip ? a.startTime : (a as BikePath).createdAt;
                            DateTime timeB = b is Trip ? b.startTime : (b as BikePath).createdAt;
                            return timeB.compareTo(timeA); // Descending
                          });
                          
                          if (combined.isEmpty) {
                            return const Center(child: Text('No recorded trips or paths found.'));
                          }
                              ...
                              if (item is Trip) {
                                return Card(
                                  ...
                                  child: ListTile(
                                    ...
                                    title: Text(item.startTime.toString().split('.')[0]), 
                                    subtitle: Text('${(item.distanceMeters / 1000).toStringAsFixed(2)} km'),
                                    trailing: TextButton(
                                ...
                                return Card(
                                  margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                                  child: ListTile(
                                    leading: const Icon(Icons.edit_road, color: Colors.green),
                                    title: Text(item.createdAt.toString().split('.')[0]),
                                    subtitle: Text('Manual Path • ${(item.distanceMeters / 1000).toStringAsFixed(2)} km'),
                                    trailing: const Chip(label: Text('Manual'), backgroundColor: Colors.green, labelStyle: TextStyle(color: Colors.white, fontSize: 10)),
                                    onTap: () {
                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Viewing manual paths is coming soon!')));
                                    },
                                  ),
                                );
                              }
                       ...
                       error: (e, s) => Center(child: Text('Error loading paths: $e')),
                     );
                   },
                   loading: () => const Center(child: CircularProgressIndicator()),
                   error: (e, s) => Center(child: Text('Error loading trips: $e')),
                 );
               },
             ),
           ),
  ...
  Widget _buildMap() {
    ...
    if (_selectedTrip != null && _selectedTrip!.points.isNotEmpty) {
      initialPos = LatLng(_selectedTrip!.points.first.latitude, _selectedTrip!.points.first.longitude);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(_searchResult != null ? 'Tap to Rate' : 'Tap Route to Report'),
      ),
      body: Stack(
        children: [
          GoogleMap(
            ...
            onMapCreated: (controller) {
              ...
              if (_selectedTrip != null && _selectedTrip!.points.isNotEmpty) {
                 controller.animateCamera(CameraUpdate.newLatLng(initialPos));
                  ...
                  controller.animateCamera(CameraUpdate.newLatLngZoom(initialPos, 16));
                  ...
                  if (_searchResult!.viewport != null) {
                    controller.animateCamera(CameraUpdate.newLatLngBounds(_searchResult!.viewport!, 50));
                  }
              }
            },
            onTap: _onMapTap,
              ...
              child: Card(
                child: Padding(
                  ...
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Text(_searchResult!.formattedAddress, style: const TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                      const SizedBox(height: 8),
                      ElevatedButton(
                        onPressed: () => _showPathQualityForm(_searchResult!.location),
...
class _ReportPathQualityDialog extends StatefulWidget {
  ...
  const _ReportPathQualityDialog({required this.position, this.routeContext, this.isFullTrip = false, required this.onSubmit});

  @override
  State<_ReportPathQualityDialog> createState() => _ReportPathQualityDialogState();
}
...
class _ReportPathQualityDialogState extends State<_ReportPathQualityDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(widget.isFullTrip ? 'Rate Entire Trip' : 'Report Path Quality'),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
            ...
            const Padding(
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Text('Report will be linked to this trip', style: TextStyle(fontSize: 12, color: Colors.blue)),
                ],
              ),
            ),
          SwitchListTile(
        ...
        Consumer(
          builder: (context, ref, child) {
            return ElevatedButton(
              onPressed: _status == null ? null : () { // Disable if no status selected
