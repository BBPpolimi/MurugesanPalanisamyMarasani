...
class _BikePathFormPageState extends ConsumerState<BikePathFormPage> {
  ...
  Future<void> _recalculateFullRoute() async {
    ...
    if (_segments.length < 2) {
      ...
      if (_segments.isNotEmpty && _mapController != null) {
         _mapController!.animateCamera(CameraUpdate.newLatLngZoom(LatLng(_segments.first.lat, _segments.first.lng), 14));
      }
      setState(() {});
      return;
    }
      ...
      try {
        ...
        if (directions != null) {
          ...
          calculatedDistance += directions.distanceValue;
             ...
             calculatedDistance += Geolocator.distanceBetween(start.lat, start.lng, end.lat, end.lng);
        }
      } catch (e) {
  ...
  void _submit() async {
    if (_segments.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Add at least one street segment.')));
      return;
    }
    
    final user = ref.read(authStateProvider).value;
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Must be logged in.')));
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Bike Path Created Successfully!')));
      }
    } catch (e) {
  ...
  void _showSearchDialog() {
     showDialog(context: context, builder: (ctx) => AlertDialog(
       ...
       content: TextField(
         ...
         decoration: const InputDecoration(hintText: 'e.g. Main St', suffixIcon: Icon(Icons.search)),
         onSubmitted: (val) {
           Navigator.pop(ctx);
           _handleSearch(val);
         },
          ...
          Expanded(
            ...
            child: Form(
              ...
              child: ListView(
                ...
                children: [
                  const Text('Path Composition', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  const SizedBox(height: 8),
                  if (_segments.isEmpty) 
                     const Padding(padding: EdgeInsets.all(8), child: Text('No streets added yet.', style: TextStyle(color: Colors.grey))),
                    ...
                    children: [
                      for (int i=0; i<_segments.length; i++)
                        ListTile(
                          ...
                          subtitle: Text(_segments[i].formattedAddress, maxLines: 1),
                          trailing: IconButton(
                  ...
                  DropdownButtonFormField<PathRateStatus>(
                    value: _status,
                    decoration: const InputDecoration(labelText: 'Overall Path Status'),
                    items: PathRateStatus.values.map((s) => DropdownMenuItem(value: s, child: Text(s.label))).toList(),
