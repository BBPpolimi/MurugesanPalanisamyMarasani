...
Within this scope we prioritized features that are architecturally central, integration heavy and material to the correctness of route ranking. Excluded or partially implemented items were those that primarily improve product completeness rather than core correctness. In particular periodic scheduled merging is operational hardening that requires scheduler configuration and concurrency safe idempotent recomputation. Password recovery and export features require additional security and platform specific handling and they do not increase confidence in the central data pipeline so they were deprioritized in favor of stability and test coverage of the core flows.
